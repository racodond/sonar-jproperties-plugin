...
public abstract class SubscriptionVisitor implements TreeVisitor {
  ...
  protected boolean isSubscribed(Tree tree) {
    return nodesToVisit.contains(((JavaPropertiesTree) tree).getKind());
  }

  private void visitChildren(Tree tree) {
    JavaPropertiesTree javaPropertiesTree = (JavaPropertiesTree) tree;

    if (!javaPropertiesTree.isLeaf()) {
      for (Iterator<Tree> iter = javaPropertiesTree.childrenIterator(); iter.hasNext();) {
        Tree next = iter.next();
